...
async fn main() -> anyhow::Result<()> {
    ...
    let token_id = U256::from_str(
        "100074770378634056601572485653252702850363676459996771990439989989293515601975",
    )?;

    let private_key = std::env::var(PRIVATE_KEY_VAR).expect("Need POLY_PRIVATE_KEY");
    let signer = LocalSigner::from_str(&private_key)?.with_chain_id(Some(polymarket_client_sdk::POLYGON));
    let safe_addr = derive_safe_wallet(signer.address(), POLYGON);
    if let Some(safe_addr) = safe_addr {
        info!("Derived Safe (funder): {:?}", safe_addr);
    } else {
        error!("Failed to derive Safe (funder)");
    }
    ...
    let market_order = client
        .market_order()
        .token_id(token_id)
        .amount(Amount::usdc(Decimal::ONE)?)
        .side(Side::Buy)
        .build()
        .await?;
